...
export class PlaceOrderPage {
    page : Page;
    creditCardLabel: Locator;
    creditCardTxtBox: Locator;
    ...
    async placeOrder(placeOrderDetails : any) {
    ...
    async searchCountryAndSelect(countryCode: string, countryName: string) {
        ...
        for (let i = 0; i < optionsCount; ++i) {
            let option : any;
            option = await optionsList.nth(i).textContent();
    ...
    async getValidateData(email: string) {
